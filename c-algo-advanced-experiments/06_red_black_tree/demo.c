 ...
 * 不变量校验。每一段打印出来的**树形和颜色**都是程序真实跑出来的(由
 * dump()/verify_or_die() 直接读 rbtree.c 的真实状态得到),不是凭算法
 * 直觉手写的"应该是这样"。
 *
 * 但要区分开:随树形一起打印的那些**解释性文字**是人写的散文,它们只能
 * 靠对照源码来保证正确,程序本身验证不了它们。第 9 节的解释文字就曾经
 * 写错过——原文说 rb_delete 是"把后继的 key 复制到 z 上,再删掉后继",
 * 而 rbtree.c 实际做的是 CLRS 的指针拼接(free 掉的是 z,后继被搬过来
 * 顶替 z 的位置)。两种写法打印出的树形和颜色一模一样,所以这个错误在
 * 输出里看不出任何异常,只能靠读源码发现。
 */
#include "rbtree.h"
#include <stdio.h>
...
int main(void) {
    ...
    printf("30 的后继(右子树最小值)是 40。rb_delete 用的是 CLRS 的指针拼接:\n");
    printf("把 40 那个节点从原来的位置摘下来、接到 30 的位置上并继承 30 的\n");
    printf("黑色,然后 free 掉 30 那个节点对象——注意被释放的是 30 而不是 40,\n");
    printf("全程没有任何一次「把 key 覆盖进另一个节点」的操作。\n");
    {
        RBTree t = rb_create();
        ...
        printf("  说明:40 原来那个红色叶子位置消失了,40 出现在原来 30 的位置\n");
        printf("  上并且是黑色——颜色来自 y->color = z->color 这一句显式赋值,\n");
        printf("  跟 40 自己原来是红色无关。颜色属于\"位置\",key 属于\"节点对象\"。\n");
        printf("  要注意:上面这段打印**区分不出**\"搬节点\"和\"复制 key\"两种实现,\n");
        printf("  它们的树形和颜色完全一样。能区分的只有节点地址(见 README 第\n");
        printf("  13 节练习 4):这里 40 这一行对应的是原来装着 40 的那个节点对象,\n");
        printf("  而装着 30 的对象已经被 free 掉了。\n");
        rb_destroy(&t);
    }
