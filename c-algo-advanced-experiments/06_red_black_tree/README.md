...
| `tests.c` | 19 个测试:基础情形、6 类 fixup case 定向构造、3 种根删除结构变体、3 组 1000+ 键随机压力测试 |
...
**真实测试结果**(19 个测试全部通过,包括三组共 4500 个键的随机压力测试):
...
`rb_delete` 处理"z 有两个孩子"时的策略(源码注释原文):
...
**关键细节**:真正被 `free` 掉的是**物理节点 y**(后继),而不是 z 原来
的节点对象——z 节点对象被保留下来,只是把它的 key 换成了 y 的 key,
颜色也保持 z 原来的颜色不变(`y->color = z->color` 这一句发生在
`transplant(tree, z, y)` 之后,把 y 接到 z 的位置上并继承 z 的颜色)。
换句话说:**留下来的是 z 的"壳"(颜色、在树里的位置),换掉的是它的
"值"(key)**。这也解释了为什么 `y_original_color`(后继原来的颜色,
不是 z 的颜色)才是决定"要不要调用 `delete_fixup`"的依据——真正从
树的叶子端被摘除的是 y 那个物理位置,黑高的变化只跟 y 原来的颜色有关。
...
30 的后继(右子树最小值)是 40。删除后:40 原来的红色叶子位置消失,
40 的 key 出现在原来 30 所在的节点位置上,并保持**黑色**(30 原来的
颜色)——40 本身的颜色和树结构由被删除的那个物理节点(z,即"30 的壳")
决定,而不是由 40 决定,与上面源码分析完全一致。
...
6. **随机压力测试要固定种子**:固定种子的伪随机序列(比如 xorshift32)
   ...
   失败一次却无法重现"。
7. **删除有两个孩子的节点时,保留的是被删节点的"壳"(颜色+位置),
   替换的是它的"值"(key)**:physically 被 `free` 的是后继节点,理解
   这一点才能正确解释"后继替位后颜色为什么不变"这个常见的困惑点。

## 12. 开发过程实录:测试到底测出了什么

如实记录:`rbtree.c` 的核心算法(旋转、插入+修复、删除+修复、性质校验)
在本次全部测试中——19 个定向构造的 case 测试、3 组共 4500 个键的随机
...
3. **删除修复的镜像对称性**:`delete_fixup` 里 x 是左孩子和 x 是右孩子
   ...
   5.3 节(左孩子版本)是完全镜像的。
4. **扩展 `rb_verify`**:目前它只校验 5 条性质。给它加上"顺便统计一下
   树里一共有多少个红色节点、多少个黑色节点",验证你的统计结果和
   `rb_black_height` 返回的黑高之间满足什么不等式关系。
5. **对照实现一个不用哨兵 NIL、直接用 NULL 的版本**(至少实现
...
`rb_verify`。19 个定向测试 + 3 组共 4500 键的随机压力测试 + 全套
sanitizer 重跑,均未发现核心算法的任何缺陷——但这个"零 bug"结论的
